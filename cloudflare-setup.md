...
| 项 | 现状 |
|----|------|
| 托管 | GitHub Pages（`WillyYe/myguilin.com`，自定义域 `myguilin.com`） |
...
1. 注册 Cloudflare 账号 → **Add a Site** → 输入 `myguilin.com` → 选 **Free** 计划。
2. Cloudflare 扫描后会给 **2 个 NS**（形如 `xxx.ns.cloudflare.com` / `yyy.ns.cloudflare.com`）。
3. 去 **阿里云（万网）控制台 → 域名 → myguilin.com → DNS 修改 / 修改 DNS 服务器**，把原来的 `dns23/24.hichina.com` 换成 Cloudflare 给的 2 个 NS。
...
| 类型 | 名称 | 目标 | 代理状态 |
|------|------|------|----------|
| CNAME | `myguilin.com` | `willyye.github.io` | 🟠 橙色云（代理） |
...
- 确认 GitHub Pages 设置里自定义域仍是 `myguilin.com`（CNAME 文件已保证）。
...
再把该 Worker **Routes** 绑定到 `myguilin.com/*` 与 `www.myguilin.com/*`（免费额度 10 万次/天，足够）。
...
```bash
curl -sI https://myguilin.com/ | grep -iE 'strict-transport|x-content|frame|referrer|permissions'
```
